...
#include <aspect/particle/interpolator/biquadratic_svd.h>
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ...
      BiquadraticSVDLeastSquares<dim>::properties_at_points(const ParticleHandler<dim> &particle_handler,
                                                      ...
                                                      const typename parallel::distributed::Triangulation<dim>::active_cell_iterator &cell) const
      {
        ...
        AssertThrow(n_particles != 0,
                    ExcMessage("At least one cell contained no particles. The 'bilinear svd'"
        ...
        // (A^TA) x = A^Tr.
        const unsigned int matrix_dimension = (dim == 2) ? 6 : 10;
            ...
            if (dim == 2)
              {
                ...
                A(positions_index, 4) = std::pow(relative_particle_position[0], 2);
                A(positions_index, 5) = std::pow(relative_particle_position[1], 2);
              }
            else
              {
        ...
        std::vector<Vector<double>> c_ATb(n_particle_properties, Vector<double>(matrix_dimension));
      ...
      template <int dim>
      void
      BiquadraticSVDLeastSquares<dim>::declare_parameters (ParameterHandler &prm)
      {
        /*
      ...
      template <int dim>
      void
      BiquadraticSVDLeastSquares<dim>::parse_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Particles");
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ASPECT_REGISTER_PARTICLE_INTERPOLATOR(BiquadraticSVDLeastSquares,
                                            "biquadratic svd least squares",
                                            "Interpolates particle properties onto a vector of points using a "
                                            "biquadratic svd least squares method. Currently only 2D models are "
