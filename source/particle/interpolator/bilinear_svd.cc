...
#include <aspect/particle/interpolator/bilinear_svd.h>
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ...
      template <int dim>
      void
      BilinearSVDLeastSquares<dim>::declare_parameters (ParameterHandler &prm)
      {
        /*
      ...
      template <int dim>
      void
      BilinearSVDLeastSquares<dim>::parse_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Particles");
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ASPECT_REGISTER_PARTICLE_INTERPOLATOR(BilinearSVDLeastSquares,
                                            "bilinear svd least squares",
                                            "Interpolates particle properties onto a vector of points using a "
                                            "bilinear svd least squares method. Currently only 2D models are "
