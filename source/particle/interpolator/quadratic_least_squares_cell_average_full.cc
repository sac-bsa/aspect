...
#include <aspect/particle/interpolator/quadratic_least_squares_cell_average_full.h>
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ...
      QuadraticLeastSquaresCellAverageFullLimiter<dim>::properties_at_points(const ParticleHandler<dim> &particle_handler,
                                                                             ...
                                                                             const typename parallel::distributed::Triangulation<dim>::active_cell_iterator &cell) const
      {
                ...
                if (use_global_values_limiter)
                  {
                    ...
                    if (original_interpolated_value != interpolated_value)
                      use_cell_average_limiter[property_index] = true;
                  }
        ...
        for (unsigned int property_index = 0; property_index < n_particle_properties; ++property_index)
          {
            if (use_cell_average_limiter[property_index])
              {
                ...
                for (auto &position : cell_properties)
                  {
                    position[property_index] = cell_average_values[property_index];
                  }
              }
          }
      ...
      template <int dim>
      void
      QuadraticLeastSquaresCellAverageFullLimiter<dim>::declare_parameters (ParameterHandler &prm)
      {
        /*
      ...
      template <int dim>
      void
      QuadraticLeastSquaresCellAverageFullLimiter<dim>::parse_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Particles");
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ASPECT_REGISTER_PARTICLE_INTERPOLATOR(QuadraticLeastSquaresCellAverageFullLimiter,
                                            "quadratic least squares cell average full limiter",
