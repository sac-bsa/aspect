...
#include <aspect/particle/interpolator/bilinear_qr.h>
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ...
      BilinearQRLeastSquares<dim>::properties_at_points(const ParticleHandler<dim> &particle_handler,
                                                        ...
                                                        const typename parallel::distributed::Triangulation<dim>::active_cell_iterator &cell) const
      {
        ...
        AssertThrow(n_particles != 0,
                    ExcMessage("At least one cell contained no particles. The 'bilinearqr'"
        ...
        AssertThrow(qr.size() == n_matrix_columns,
                    ExcMessage("The matrix A was rank deficent during bilinearqr least squares interpolation."));
      ...
      template <int dim>
      void
      BilinearQRLeastSquares<dim>::parse_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Particles");
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ASPECT_REGISTER_PARTICLE_INTERPOLATOR(BilinearQRLeastSquares,
                                            "bilinear qr least squares",
                                            "Interpolates particle properties onto a vector of points using a "
                                            "bilinear qr least squares method. Note that deal.II must be configured"
                                            " with BLAS/LAPACK.")
    }
  }
}
