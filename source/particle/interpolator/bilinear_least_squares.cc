    ...
    namespace Interpolator
    {
                                                      ...
                                                      const typename parallel::distributed::Triangulation<3>::active_cell_iterator &cell) const
      {
             ...
             particle != particle_range.end(); ++particle, ++index)
        {
          ...
          const Point<3> particle_position = particle->get_location();
          A(index, 0) = 1;
          A(index, 1) = (particle_position[0] - approximated_cell_midpoint[0])/cell_diameter;
          A(index, 2) = (particle_position[1] - approximated_cell_midpoint[1])/cell_diameter;
          A(index, 3) = (particle_position[2] - approximated_cell_midpoint[2])/cell_diameter;
          A(index, 4) = (particle_position[0] - approximated_cell_midpoint[0]) * (particle_position[1] - approximated_cell_midpoint[1])/std::pow(cell_diameter,2);
          A(index, 5) = (particle_position[0] - approximated_cell_midpoint[0]) * (particle_position[2] - approximated_cell_midpoint[2])/std::pow(cell_diameter, 2);
          A(index, 6) = (particle_position[1] - approximated_cell_midpoint[1]) * (particle_position[2] - approximated_cell_midpoint[2])/std::pow(cell_diameter, 2);
        }
        ...
        for (typename std::vector<Point<3> >::const_iterator itr = positions.begin(); itr != positions.end(); ++itr, ++index_positions)
        {
          const Point<3> support_point = *itr;
          double interpolated_value = c[0] +
                                      c[1]*(support_point[0] - approximated_cell_midpoint[0])/cell_diameter +
                                      c[2]*(support_point[1] - approximated_cell_midpoint[1])/cell_diameter +
                                      c[3]*(support_point[2] - approximated_cell_midpoint[2])/cell_diameter +
                                      c[4]*(support_point[0] - approximated_cell_midpoint[0])*(support_point[1] - approximated_cell_midpoint[1])/std::pow(cell_diameter,2) +
                                      c[5]*(support_point[0] - approximated_cell_midpoint[0])*(support_point[2] - approximated_cell_midpoint[2])/std::pow(cell_diameter,2) +
                                      c[6]*(support_point[1] - approximated_cell_midpoint[1])*(support_point[2] - approximated_cell_midpoint[2])/std::pow(cell_diameter,2);
