  ...
  namespace Particle
  {
    namespace Interpolator
    {
                                                 ...
                                                 const typename parallel::distributed::Triangulation<dim>::active_cell_iterator &cell) const
      {
        ...
        for (unsigned int position_index=0; position_index < positions.size(); ++position_index)
          {
            ...
            if (n_particles > 0)
              {
                ...
                for (typename ParticleHandler<dim>::particle_iterator particle = particle_range.begin();
                  ...
                  {
                    const double dist = (positions[position_index] - particle->get_location()).norm_square();
                ...
                for (unsigned int i = 0; i < n_particle_properties; ++i)
                  if (selected_properties[i])
                    point_properties[position_index][i] = neighbor_props[i];
              }
            else
              {
                ...
                for (unsigned int i=0; i<neighbors.size(); ++i)
                  {
                    ...
                    const double dist = (positions[position_index] - neighbors[i]->center()).norm_square();
                    if (dist < minimum_distance)
                      {
                        minimum_distance = dist;
                ...
                if (nearest_neighbor_cell != numbers::invalid_unsigned_int)
                  {
                    point_properties[position_index] = properties_at_points(particle_handler,
                                                                            std::vector<Point<dim> > (1,positions[position_index]),
                                                                            selected_properties,
                ...
                else if (allow_cells_without_particles && nearest_neighbor_cell == numbers::invalid_unsigned_int)
                  {
                    for (unsigned int i = 0; i < n_particle_properties; ++i)
                      if (selected_properties[i])
                        point_properties[position_index][i] = 0;
                  }
