...
#include <aspect/particle/interpolator/biquadratic_qr.h>
    ...
    namespace Interpolator
    {
                                                        ...
                                                        const typename parallel::distributed::Triangulation<dim>::active_cell_iterator &cell) const
      {
        ...
        AssertThrow(n_particles != 0,
                    ExcMessage("At least one cell contained no particles. The 'biquadraticqr'"
            ...
            if (dim == 2)
              {
                ...
                A[4][positions_index] = std::pow(relative_particle_position[0], 2);
                A[5][positions_index] = std::pow(relative_particle_position[1], 2);
              }
            else
              {
                ...
                A[7][positions_index] = std::pow(relative_particle_position[0], 2);
                A[8][positions_index] = std::pow(relative_particle_position[1], 2);
                A[9][positions_index] = std::pow(relative_particle_position[2], 2);
              }
          }
        ...
        AssertThrow(qr.size() == n_matrix_columns,
                    ExcMessage("The matrix A was rank deficent during biquadraticqr least squares interpolation."));
      ...
      template <int dim>
      void
      BiquadraticQRLeastSquares<dim>::parse_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Particles");
...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ASPECT_REGISTER_PARTICLE_INTERPOLATOR(BiquadraticQRLeastSquares,
                                            "biquadratic qr least squares",
                                            "Interpolates particle properties onto a vector of points using a "
                                            "biquadratic qr least squares method. Note that deal.II must be configured"
                                            " with BLAS/LAPACK.")
    }
  }
}
